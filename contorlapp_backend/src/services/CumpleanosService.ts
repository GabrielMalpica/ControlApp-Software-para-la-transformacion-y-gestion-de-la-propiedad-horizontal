...
export class CumpleanosService {
  ...
  private async obtenerUsuarioActor(actorUserId: string) {
  ...
  async obtenerEmpresaIdActor(actorUserId: string): Promise<string> {
  ...
  private async listarCumpleanosEmpresa(empresaId: string, mes: number) {
    const rows = await this.db.$queryRaw<CumpleaneroRow[]>(Prisma.sql`
      SELECT DISTINCT
        u."id",
        u."nombre",
        u."correo",
  ...
  async listarCumpleanosAnioActor(actorUserId: string): Promise<CumpleaneroItem[]> {
    ...
    const items = await Promise.all(
      Array.from({ length: 12 }, (_, index) =>
        this.listarCumpleanosEmpresa(empresaId, index + 1),
      ),
    );
  ...
  async cumpleanosHoyActor(actorUserId: string) {
    const actor = await this.obtenerUsuarioActor(actorUserId);
    const hoy = this.hoyBogota();
    const fecha = new Date(actor.fechaNacimiento as unknown as Date);
    const bogotaFecha = new Date(
      fecha.toLocaleString("en-US", { timeZone: "America/Bogota" }),
    );
    const esCumpleanosHoy =
      bogotaFecha.getDate() === hoy.dia && bogotaFecha.getMonth() + 1 === hoy.mes;
  ...
  private async existeNotificacionCumpleHoy(
    usuarioId: string,
    tipo: string,
    birthdayUserId: string,
  ) {
    const rows = await this.db.$queryRaw<Array<{ total: number }>>(Prisma.sql`
  ...
  async asegurarNotificacionesCumpleanosHoy(actorUserId: string) {
    const empresaId = await this.obtenerEmpresaIdActor(actorUserId);
    const hoy = this.hoyBogota();
    ...
    const destinatariosGestion = Array.from(
      new Set([...gerentes.map((x) => x.id), ...jefes.map((x) => x.id)]),
    );

    for (const persona of deHoy) {
      for (const usuarioId of destinatariosGestion) {
        const existe = await this.existeNotificacionCumpleHoy(
          usuarioId,
          "CUMPLEANOS_EQUIPO",
          persona.id,
        );
      ...
      const existePersonal = await this.existeNotificacionCumpleHoy(
        persona.id,
        "CUMPLEANOS_PERSONAL",
        persona.id,
      );
