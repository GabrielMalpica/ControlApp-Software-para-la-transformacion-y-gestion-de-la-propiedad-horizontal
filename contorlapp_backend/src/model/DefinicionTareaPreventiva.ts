...
export const CrearDefinicionPreventivaDTO = z.object({
...
.refine((d) => {
  ...
  const tieneRendimiento = d.unidadCalculo && d.areaNumerica !== undefined && d.rendimientoBase !== undefined;
  const tieneDuracionFija = d.duracionHorasFija !== undefined;
  return tieneRendimiento || tieneDuracionFija;
}, {
  message: "Debe indicar (unidadCalculo + areaNumerica + rendimientoBase) o duracionHorasFija.",
});
...
.refine((d) => {
  const algunoA = d.unidadCalculo !== undefined || d.areaNumerica !== undefined || d.rendimientoBase !== undefined;
  if (!algunoA) return true;
  // Si toca A, que vengan los tres (o null para limpiar)
  const okA = d.unidadCalculo !== undefined && d.areaNumerica !== undefined && d.rendimientoBase !== undefined;
  return okA;
}, {
  message: "Si modifica cálculo por rendimiento, incluya unidadCalculo, areaNumerica y rendimientoBase.",
});
...
>(row: T): DefinicionTareaPreventivaPublica {
  // Nota: los Decimal de Prisma se exponen aquí como number en tu capa pública;
  // haz la conversión en services (Prisma.Decimal) al escribir en DB.
 ...
 * Útil en servicios antes de generar bloques.
 */
