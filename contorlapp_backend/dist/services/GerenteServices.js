...
const ConjuntoExcelTemplateService_1 = require("./ConjuntoExcelTemplateService");
const ConjuntoCargaMasivaService_1 = require("./ConjuntoCargaMasivaService");
...
class GerenteService {
    ...
    async obtenerEmpresaIdParaCambioRol(tx, userId) {
        ...
        return (gerente?.empresaId ??
            jefe?.empresaId ??
            supervisor?.empresaId ??
            operario?.empresaId ??
            (await this.resolverEmpresaNit()));
    }
    async validarCambioRolSeguro(tx, userId, currentRole, nextRole) {
        if (currentRole === nextRole)
            return;
        if (currentRole === client_1.Rol.administrador) {
            const total = await tx.conjunto.count({ where: { administradorId: userId } });
            if (total > 0) {
                throw new Error('No se puede cambiar el rol de este administrador porque tiene conjuntos asignados.');
            }
        }
        if (currentRole === client_1.Rol.supervisor) {
            ...
            if (tareasActivas > 0 || preventivasActivas > 0) {
                throw new Error('No se puede cambiar el rol de este supervisor porque tiene tareas o preventivas activas asignadas.');
            }
        }
        if (currentRole === client_1.Rol.operario) {
            const [tareasActivas, preventivasActivas, conjuntosAsignados, solicitudesMaquinaria, maquinariaAsignada,] = await Promise.all([
                tx.tarea.count({
            ...
            if (tareasActivas > 0 ||
                ...
                throw new Error('No se puede cambiar el rol de este operario porque tiene asignaciones activas, conjuntos, solicitudes o maquinaria asociada.');
            }
        }
    }
    async sincronizarPerfilRol(tx, userId, currentRole, nextRole) {
        if (currentRole === nextRole)
            return;
    ...
    validarDisponibilidadOperarioPeriodos(periodos) {
        ...
        if (invalido) {
            throw new Error("Si el operario trabaja domingo, debe tener un dia de descanso entre semana en ese periodo.");
        }
    }
    ...
    async crearUsuario(payload) {
        ...
        if (existeId)
            throw new Error("Ya existe un usuario con esa cÃƒÂ©dula.");
        if (existeCorreo)
            throw new Error("Ya existe un usuario con ese correo.");
    ...
    async editarUsuario(id, payload) {
        ...
        return this.prisma.usuario.findUnique({ where: { id }, select: Usuario_1.usuarioPublicSelect });
    }
    async asignarGerente(payload) {
        ...
        if (!empresa)
            throw new Error("Ã¢ÂÅ’ Empresa no encontrada con ese NIT.");
        if (!usuario)
            throw new Error("Ã¢ÂÅ’ Usuario no encontrado.");
    ...
    async asignarJefeOperaciones(payload) {
        ...
        if (!empresa)
            throw new Error("Ã¢ÂÅ’ No hay empresa registrada.");
        if (!usuario)
            throw new Error("Ã¢ÂÅ’ Usuario no encontrado.");
    ...
    async asignarOperario(payload) {
        ...
        return this.prisma.operario.create({
            ...
            select: Operario_1.operarioPublicSelect,
        });
    }
    ...
    async generarPlantillaConjunto() {
        const empresaId = await this.resolverEmpresaNit();
        ...
        return new ConjuntoExcelTemplateService_1.ConjuntoExcelTemplateService().generar({
            insumos,
            herramientas,
    ...
    async cargarConjuntoMasivo(_actorUserId, file) {
        const empresaId = await this.resolverEmpresaNit();
        return new ConjuntoCargaMasivaService_1.ConjuntoCargaMasivaService(this.prisma, (client, dto, currentEmpresaId, administradorId) => this.crearConjuntoConEstructura(client, dto, currentEmpresaId, administradorId)).cargar(empresaId, file);
    }
    ...
    async eliminarConjunto(conjuntoId, options = {}) {
        ...
        if (maquinariaActivaEnConjunto.length > 0)
            throw new Error("Ã¢ÂÅ’ El conjunto tiene maquinaria activa asignada (propia o prestada).");
        const [inventario, totalInsumosInventario, totalConsumos, totalSolicitudesInsumo, totalSolicitudesMaquinaria, totalSolicitudesTarea, totalAsignacionesMaquinaria] = await Promise.all([
        ...
        if (dependencias.length > 0 && !confirmar) {
            return {
                ...
                message: "El conjunto todavia tiene informacion relacionada. Si deseas borrarlo de todas formas, repite la solicitud con ?confirmar=true.",
                details: {
                    conjuntoId,
    ...
    async agregarInsumoAConjunto(payload) {
        ...
        if (!inventario)
            throw new Error(`Ã¢ÂÅ’ No se encontrÃƒÂ³ inventario para el conjunto ${dto.conjuntoId}`);
    ...
    async agregarInsumoAlCatalogo(payload, empresaId) {
        ...
        if (existe)
            throw new Error("Ã°Å¸Å¡Â« Ya existe un insumo con ese nombre y unidad en el catÃƒÂ¡logo.");
    ...
    buildBloqueoNoReemplazableMessage(params) {
    ...
    async buscarSugerenciaMismoDia(params) {
        ...
        const bloques = (0, schedulerUtils_1.buscarHuecoDiaConSplitEarliest)({
            ...
            bloqueos,
            desiredStartMin: (0, schedulerUtils_1.toMinOfDaySafe)(inicio),
            maxBloques: 2,
        });
        if (!bloques)
            return null;
    ...
    async resolverOrigenHerramienta(params) {
        const { tx, empresaId, conjuntoId, herramientaId, cantidad, inicio, fin, tareaExcluidaId, } = params;
        const [stockConjunto, stockEmpresa, reservas] = await Promise.all([
            ...
            tx.empresaHerramientaStock.findUnique({
                where: {
            ...
            tx.usoHerramienta.findMany({
                where: {
                    ...
                    tarea: {
                        estado: {
                            notIn: ["COMPLETADA", "NO_COMPLETADA", "CANCELADA"],
                        },
                    },
                },
        ...
        const totalConjunto = stockConjunto.reduce((acc, row) => acc + Number(row.cantidad), 0);
        ...
        if (disponibleConjunto >= cantidad)
            return "CONJUNTO";
        if (disponibleEmpresa >= cantidad)
            return "EMPRESA";
        throw new Error(`HERRAMIENTA_SIN_STOCK_${herramientaId}_${disponibleConjunto}_${disponibleEmpresa}`);
    }
    async reservarHerramientasTarea(params) {
    ...
    async listarHuecosReprogramacionEnMes(params) {
        ...
        for (let guard = 0; guard < Math.max(1, limit) * 3; guard++) {
            ...
            if (!slot)
                break;
            const duplicate = out.some((item) => item.fechaInicio.getTime() === slot.fechaInicio.getTime() &&
    ...
    async asignarTarea(payload) {
        ...
        const maquinariaIds = Array.isArray(dto.maquinariaIds)
            ...
            : [];
        const herramientas = Array.isArray(dto.herramientas)
            ? dto.herramientas
                .map((x) => ({
                herramientaId: Number(x?.herramientaId),
                cantidad: Number(x?.cantidad ?? 1),
            }))
                .filter((x) => Number.isFinite(x.herramientaId) &&
                x.herramientaId > 0 &&
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (dto.conjuntoId && operariosIds.length) {
                ...
                if (choqueOperario) {
                    ...
                    return {
                        ok: false,
                        reason: "HAY_SOLAPE_CON_TAREAS_EXISTENTES",
                        message: "Ese horario ya estÃƒÂ¡ ocupado y no se encontrÃƒÂ³ hueco en el dÃƒÂ­a.",
                    };
                }
            }
            ...
            if (dto.conjuntoId && maquinariaIds.length) {
                ...
                for (const maqId of maquinariaIds) {
                    ...
                    if (choque) {
                        throw new Error(`MAQUINARIA_OCUPADA: maquinaria ${maqId} ya estÃƒÂ¡ reservada`);
                    }
                    // Crear uso
    ...
    async crearCorrectivaConReglas(payload) {
        ...
        const soloAvisoAuto = opciones.autoOptions.length > 0 && opciones.confirmOptions.length === 0;
    ...
    async asignarTareaConReemplazoV2(payload) {
        ...
        const { tarea: dto, reemplazarIds, accionReemplazadas, motivoReemplazo, motivo, reprogramaciones, } = parsed.data;
        const motivoUsuario = (motivoReemplazo ?? motivo ?? "").trim() || null;
        const reprogramacionesMap = new Map((reprogramaciones ?? []).map((item) => [item.tareaId, item]));
        const tipo = String(dto.tipo ?? "CORRECTIVA").toUpperCase();
        const prioridad = Number(dto.prioridad ?? 2);
        if (tipo !== "CORRECTIVA") {
            return {
        ...
        return this.prisma.$transaction(async (tx) => {
            ...
            if (tareasReemplazar.length !== reemplazarIds.length) {
                return {
                    ...
                    message: "Una o mÃƒÂ¡s tareas a reemplazar no pertenecen al conjunto indicado.",
                };
            }
            ...
            if (requiereConfirmacion && !accionReemplazadas) {
                return {
                    ...
                    message: "Debe indicar si las preventivas reemplazadas se reprograman o se marcan como no completadas.",
                };
            }
            if (accionReemplazadas === "REPROGRAMAR") {
                const opcionesReprogramacion = await Promise.all(tareasReemplazar.map(async (t) => {
                ...
                const faltanSelecciones = tareasReemplazar.some((t) => !reprogramacionesMap.has(t.id));
                if (faltanSelecciones) {
                    return {
                        ok: false,
                        needsReprogrammingSelection: true,
                        message: "Selecciona una nueva franja horaria para cada preventiva reemplazada.",
                ...
                for (const t of tareasReemplazar) {
                    ...
                    const coincide = opcionesReprogramacion
                        ...
                        ?.slots.some((slot) => slot.fechaInicio.getTime() === seleccion.fechaInicio.getTime() &&
                        slot.fechaFin.getTime() === seleccion.fechaFin.getTime());
                    if (!coincide) {
                        return {
                            ok: false,
                            needsReprogrammingSelection: true,
                            message: "Una de las franjas elegidas ya no esta disponible. Selecciona nuevamente el horario de reprogramacion.",
                            opcionesReprogramacion,
                        };
                    }
                }
                ...
                if (accionFinal === "REPROGRAMAR") {
                    ...
                    await tx.tarea.update({
                        ...
                        data: {
                            ...
                            duracionMinutos: Math.max(1, Math.round((seleccion.fechaFin.getTime() - seleccion.fechaInicio.getTime()) /
                                60000)),
                            reprogramada: true,
    ...
    async editarTarea(tareaId, payload) {
        ...
        const mergedDuracion = dto.duracionMinutos ??
            (dto.duracionHoras
                ? Math.max(1, Math.round(dto.duracionHoras * 60))
        ...
        const mergedOperariosIds = dto.operariosIds?.map((id) => id.toString()) ??
