...
exports.AgendaMaquinariaService = void 0;
const DIAS_ENTREGA_RECOGIDA = new Set([1, 3, 6]); // Lun(1), Mie(3), Sab(6)
class AgendaMaquinariaService {
    constructor(prisma) {
        this.prisma = prisma;
    }
    // =========================
    ...
    // Logística (L, X, S)
    // =========================
    buscarDiaPermitidoAnterior(fecha, diasPermitidos) {
    ...
    buscarDiaPermitidoPosterior(fecha, diasPermitidos) {
        const d = this.dayOnly(fecha);
    ...
    calcularRangoReserva(usoIni, usoFin) {
        const ini = +usoIni <= +usoFin ? usoIni : usoFin;
        const fin = +usoIni <= +usoFin ? usoFin : usoIni;
        const entregaDia = this.buscarDiaPermitidoAnterior(ini, DIAS_ENTREGA_RECOGIDA);
        const recogidaDia = this.buscarDiaPermitidoPosterior(fin, DIAS_ENTREGA_RECOGIDA);
        return {
            entregaDia,
            recogidaDia,
    ...
    fillGridEAPR(params) {
        ...
        const mark = (d, letter) => {
            ...
            if (col < 0 || col > 5)
                return; // domingo no se pinta
            // prioridad: E/R no se pisan
            if ((grid[col] === "E" || grid[col] === "R") &&
                (letter === "P" || letter === "A"))
                return;
    ...
    // =========================================================
    async agendaGlobalPorMaquina(params) {
        ...
        const data = maquinas.map((m) => {
            ...
            const items = Array.from(groups.entries()).flatMap(([groupKey, arr]) => {
                ...
                if (gpKey && tareasPorGp.has(gpKey)) {
                    const ts = tareasPorGp.get(gpKey);
                    const diasUnicos = new Set(ts
                        .map((t) => t.fechaInicio)
                        .filter((d) => d instanceof Date && !isNaN(+d))
                        .map((d) => this.isoDate(this.dayOnly(d)))).size;
                ...
                // ---- logística
                const { entregaDia, recogidaDia } = this.calcularRangoReserva(usoIni, usoFin);
                ...
                for (let semana = semanaIni; semana <= semanaFin; semana++) {
                    const { start: wStart, end: wEnd } = this.weekRange(anio, mes, semana);
...
exports.AgendaMaquinariaService = AgendaMaquinariaService;
