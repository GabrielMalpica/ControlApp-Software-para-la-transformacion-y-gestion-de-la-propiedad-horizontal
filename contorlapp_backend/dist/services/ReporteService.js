...
function buildCompromisoAns(compromiso, referenceDate = new Date()) {
...
class ReporteService {
    ...
    async compromisosDashboard(payload) {
        const { desde, hasta, conjuntoId } = RangoConConjuntoOpcionalDTO.parse(payload);
        ...
        const days = buildDayRange(desde, hasta);
        const createdSeries = Object.fromEntries(days.map((d) => [d, 0]));
        const closedSeries = Object.fromEntries(days.map((d) => [d, 0]));
        const porAns = {
            verde: 0,
            naranja: 0,
            rojo: 0,
        ...
        for (const item of items) {
            ...
            if (item.completado) {
                ...
                row.abiertos += 1;
                if (ans.ansEstado === "verde")
                    row.verdes += 1;
                if (ans.ansEstado === "naranja")
                    row.naranjas += 1;
                if (ans.ansEstado === "rojo")
                    row.rojos += 1;
            }
        ...
        const promedioDiasCierre = cerradosConFecha > 0
            ? Number((totalDiasCierre / cerradosConFecha).toFixed(1))
            : 0;
        const promedioDiasAbiertos = abiertos > 0 ? Number((totalDiasAbiertos / abiertos).toFixed(1)) : 0;
        const porcentajeCumplimiento = total > 0 ? Math.round((porEstado.cerrados / total) * 100) : 0;
        const porcentajeAnsSaludable = porEstado.abiertos > 0
    ...
    async reporteMensualDetalle(payload) {
        ...
        const tareas = await this.prisma.tarea.findMany({
            ...
            select: {
                ...
                reprogramada: true,
                reprogramadaEn: true,
                reprogramadaMotivo: true,
                reprogramadaPorTareaId: true,
        ...
        const tareaIdsReemplazo = Array.from(new Set(tareasPreventivasReemplazadas
            .map((t) => t.reprogramadaPorTareaId)
            .filter((id) => typeof id === "number" && id > 0)));
        const tareasReemplazo = tareaIdsReemplazo.length > 0
        ...
        const reemplazosPreventivaPorCorrectiva = tareasPreventivasReemplazadas.map((t) => {
            const tareaReemplazoId = t.reprogramadaPorTareaId;
