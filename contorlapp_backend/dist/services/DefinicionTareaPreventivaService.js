...
const ReasignarOperarioBorradorDTO = zod_1.z.object({
    conjuntoId: zod_1.z.string().min(3),
    tareaId: zod_1.z.number().int().positive(),
...
const GestionarBloqueExcluidaDTO = zod_1.z.object({
    conjuntoId: zod_1.z.string().min(3),
    excluidaId: zod_1.z.number().int().positive(),
    bloqueId: zod_1.z.string().min(1),
    fechaInicio: zod_1.z.coerce.date().optional(),
    fechaFin: zod_1.z.coerce.date().optional(),
});
const ReordenarTareasDiaBorradorDTO = zod_1.z.object({
    conjuntoId: zod_1.z.string().min(3),
    fecha: zod_1.z.coerce.date(),
    tareaIds: zod_1.z.array(zod_1.z.number().int().positive()).min(2),
});
/* =========================================================
 * Service
 * ======================================================= */
class DefinicionTareaPreventivaService {
    ...
    validarProgramacionFrecuencia(params) {
        ...
        if ((frecuencia === client_1.Frecuencia.BIMESTRAL ||
            frecuencia === client_1.Frecuencia.TRIMESTRAL ||
            frecuencia === client_1.Frecuencia.SEMESTRAL ||
            frecuencia === client_1.Frecuencia.ANUAL) &&
            !(fechasProgramadasJson?.length)) {
            throw new Error("Esta frecuencia requiere al menos una fecha programada seleccionada desde el calendario.");
        }
        ...
        if (requeridas != null) {
            ...
            if (actuales < requeridas) {
                throw new Error(`Faltan ${requeridas - actuales} fecha(s) para completar la frecuencia ${frecuencia}.`);
            }
            if (actuales > requeridas) {
                throw new Error(`No puedes registrar más de ${requeridas} fecha(s) para la frecuencia ${frecuencia}.`);
            }
        }
    }
    fechasRequeridasPorFrecuencia(frecuencia) {
        switch (frecuencia) {
    ...
    metadataAsObject(value) {
        return value && typeof value === "object" && !Array.isArray(value)
            ? ({ ...value })
            : {};
    }
    leerDivisionManualExcluida(value) {
        ...
        for (let index = 0; index < bloquesRaw.length; index++) {
            ...
            bloques.push({
                ...
                tareaProgramadaId: block.tareaProgramadaId == null ? null : Number(block.tareaProgramadaId),
    ...
    construirMetadataConDivisionManual(base, division) {
    ...
    resolverBloqueDivision(division, bloqueId) {
        if (!division?.activa)
            return null;
    ...
    async sugerirHuecosParaExcluidaCore(params) {
        const { conjuntoId, excluida, fechaPreferida, maxOpciones = 8, mismoDiaPrimero = true, permitirSplitMismoDia = true, permitirDivisionFlexible = true, } = params;
        ...
        const opciones = [];
        const pushOpcion = (bloquesPlan) => {
            if (!bloquesPlan.length || opciones.length >= maxOpciones)
                return;
        ...
        for (const dia of fechas) {
            ...
            if (bloques?.length) {
                const bloquesPlan = bloques.map((bloque) => ({
                    fechaInicio: (0, schedulerUtils_1.toDateAtMin)(dia, bloque.i),
                    fechaFin: (0, schedulerUtils_1.toDateAtMin)(dia, bloque.f),
                }));
                try {
            ...
            if (!permitirDivisionFlexible || opciones.length >= maxOpciones)
                continue;
            ...
            if (planDividido.length)
                pushOpcion(planDividido);
        }
        return {
    ...
    async construirPlanFlexibleExcluida(params) {
        ...
        for (let idx = startIndex; idx < fechas.length && restante > 0; idx++) {
            ...
            if (excluida.operariosIds.length) {
                ...
                ocupadosGlobal = (0, schedulerUtils_1.mergeIntervalos)(bloqueos.map((bloqueo) => ({ i: bloqueo.startMin, f: bloqueo.endMin })));
            }
            const blocked = (0, schedulerUtils_1.mergeIntervalos)([
                ...ocupadosGlobal,
    ...
    async materializarExcluidaEnBloques(params) {
        ...
        const bloquesOrdenados = [...params.bloques].sort((a, b) => a.fechaInicio.getTime() - b.fechaInicio.getTime());
        const duracionTotal = bloquesOrdenados.reduce((acc, bloque) => acc + Math.max(1, Math.round((bloque.fechaFin.getTime() - bloque.fechaInicio.getTime()) / 60000)), 0);
    ...
    async actualizar(conjuntoId, id, payload) {
        ...
        const actual = await this.prisma.definicionTareaPreventiva.findUnique({
            where: { id },
            select: {
                frecuencia: true,
        ...
        this.validarProgramacionFrecuencia({
            ...
            diaMesProgramado: dto.diaMesProgramado === undefined ? actual.diaMesProgramado : dto.diaMesProgramado,
            fechasProgramadasJson: dto.fechasProgramadasJson === undefined
                ? (actual.fechasProgramadasJson ?? null)
                : dto.fechasProgramadasJson,
        });
        ...
        for (const c of confirmacionesReemplazo) {
            ...
            confirmacionesMap.set(keyConfirmacion(Number(c.defId), String(c.fecha), Number(c.prioridadSolicitante ?? 0), Number(c.prioridadObjetivo ?? 0)), {
                ...
                candidataId: c.candidataId != null && Number.isFinite(Number(c.candidataId))
                    ...
                    : undefined,
                reprogramarReemplazada: c.reprogramarReemplazada == null
                    ? undefined
                        ...
                        if (prioridad === 1 || prioridad === 2) {
                            ...
                            if (prioridad === 1) {
                                ...
                                if (confirmP2?.aceptar === true && candidatasP2.length) {
                                    ...
                                    const repConfirmadoP2 = await (0, schedulerUtils_1.intentarReemplazoPorPrioridadBaja)({
                                        ...
                                        marcarReemplazadasComoNoCompletadas: confirmP2.reprogramarReemplazada === false,
                                        incluirBorradorEnAgenda: true,
                                        incluirPublicadasEnAgenda,
                                        onEvent: (ev) => {
                                            if (ev.tipo === "REEMPLAZO" &&
                            ...
                            if (confirmP3?.aceptar === true && candidatasP3.length) {
                                ...
                                const repConfirmadoP3 = await (0, schedulerUtils_1.intentarReemplazoPorPrioridadBaja)({
                                    ...
                                    prioridadesCandidatas: [3],
                                    candidatasIdsPreferidas: candidatasPreferidas,
                                    marcarReemplazadasComoNoCompletadas: confirmP3.reprogramarReemplazada === false,
                                    incluirBorradorEnAgenda: true,
                                    incluirPublicadasEnAgenda,
                                    onEvent: (ev) => {
                                        if (ev.tipo === "REEMPLAZO" &&
    ...
    async editarBloqueBorrador(conjuntoId, tareaId, payload) {
        ...
        if (fechaInicio && fechaFin && operariosIdsFinal.length) {
            const solapes = await Promise.all(operariosIdsFinal.map(async (opId) => ({
                opId,
        ...
        if (fechaInicio && fechaFin) {
            ...
            if (maqIds.length) {
                ...
                if (disponibilidad.ok) {
                    const conflictos = (disponibilidad.conflictos ?? []).filter((item) => maqIds.includes(item.maquinariaId));
    ...
    async dividirExcluidaManual(payload) {
        ...
        if (actual?.bloques.some((bloque) => bloque.estado === "AGENDADO")) {
            throw new Error("La tarea ya tiene bloques agendados. No puedes redefinir la división manual en este momento.");
        }
    ...
    async sugerirHuecosBloqueExcluida(payload) {
    ...
    async agendarBloqueExcluida(payload) {
        const dto = GestionarBloqueExcluidaDTO.parse(payload);
        const excluida = await this.prisma.preventivaExcluidaBorrador.findUnique({
            where: { id: dto.excluidaId },
        });
        ...
        const tarea = await this.prisma.$transaction(async (tx) => {
            const creada = await tx.tarea.create({
                data: {
                    ...
                    fechaInicio,
                    fechaFin,
                    duracionMinutos: Math.max(1, Math.round((fechaFin.getTime() - fechaInicio.getTime()) / 60000)),
                    prioridad: excluida.prioridad,
                    estado: client_1.EstadoTarea.ASIGNADA,
                    tipo: client_1.TipoTarea.PREVENTIVA,
                    frecuencia: excluida.frecuencia,
            ...
            const nuevaDivision = {
                ...division,
                actualizadaEn: new Date().toISOString(),
                bloques: division.bloques.map((item) => item.id === dto.bloqueId
                    ? {
                        ...item,
                        estado: "AGENDADO",
    ...
    async reordenarTareasBorradorDia(payload) {
        ...
        for (const tarea of seleccionOrdenada) {
            ...
            const segmentos = intentarDistribuirDuracionReordenamiento({
                fecha: dto.fecha,
            ...
            if (operariosIds.length) {
                ...
                for (const segmento of segmentos) {
                    ...
                    if (solape) {
                        throw new Error(construirMensajeSolapeReordenamiento({
                            tareaActual: tarea,
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            for (const item of recreaciones) {
                ...
                for (const segmento of item.segmentos) {
                    await tx.tarea.create({
                        data: buildTareaBorradorCreateData(item.original, segmento.fechaInicio, segmento.fechaFin),
                    });
                }
            }
        });
    ...
    async agendarExcluidaBorrador(payload) {
        ...
        if ((this.leerDivisionManualExcluida(excluidaActual.metadataJson)?.bloques.length ?? 0) > 0) {
            throw new Error("Esta tarea ya fue dividida manualmente. Agenda cada bloque por separado desde el desplegable.");
        }
        let fechaInicio = dto.fechaInicio ?? null;
        let fechaFin = dto.fechaFin ?? null;
    ...
    async listarMaquinariaDisponible(params) {
        const { conjuntoId, fechaInicioUso, fechaFinUso, excluirTareaId, excluirGrupoPlanId, } = params;
        ...
        for (const g of gruposBorrador.values()) {
            ...
            const solapeUsoReal = overlaps(fechaInicioUso, fechaFinUso, g.usoIni, g.usoFin);
        ...
        const conjuntoIds = Array.from(new Set([conjuntoId]
            .concat(ocupadasDetalle
        ...
        const descripcionSolicitada = excluirTareaId != null
            ? ((await this.prisma.tarea.findUnique({
                where: { id: excluirTareaId },
                select: { descripcion: true },
            }))?.descripcion ?? "Tarea reprogramada")
            : "Tarea solicitada";
        const conflictos = ocupadasDetalleConNombre.map((item) => buildConflictoMaquinariaDetalle({
            ...
            tipoSolape: item.fuente === "BORRADOR_PREVENTIVA"
                ? "BORRADOR_INTERNO"
                : (item.conjuntoId ?? null) === conjuntoId
                    ? "USO_REAL"
                    : "RESERVA_LOGISTICA",
            motivo: item.fuente === "BORRADOR_PREVENTIVA"
    ...
    async crearReservasPlanificadasParaTareas(params) {
            ...
            for (let j = i + 1; j < plan.length; j++) {
                ...
                for (const maquinariaId of a.maqIds) {
                    if (!maqSetB.has(maquinariaId))
                        continue;
                    conflictosInternos.push(buildConflictoMaquinariaDetalle({
                        maquinariaId,
                        tareaSolicitada: {
                        ...
                        motivo: "La maquina ya esta planificada en otro bloque preventivo del mismo borrador y ambos usos reales se cruzan.",
                    }));
                }
            }
        }
        ...
        for (const p of plan) {
            for (const maquinariaId of p.maqIds) {
                ...
                for (const u of ocup) {
                    ...
                    conflictos.push(buildConflictoMaquinariaDetalle({
                        maquinariaId,
                        tareaSolicitada: {
        ...
        if (conflictos.length) {
            ...
            const conjuntoIds = Array.from(new Set(conflictos
            ...
            const conjuntos = conjuntoIds.length
                ...
                : [];
            const conjuntoNombrePorId = new Map(conjuntos.map((item) => [item.nit, item.nombre]));
            const conflictosEnriquecidos = conflictos.map((item) => ({
                ...item,
                maquinaNombre: nombrePorId.get(item.maquinariaId) ?? item.maquinaNombre ?? null,
                tareaSolicitada: {
                    ...item.tareaSolicitada,
                    conjuntoNombre: item.tareaSolicitada.conjuntoId == null
                        ? null
                        : (conjuntoNombrePorId.get(item.tareaSolicitada.conjuntoId) ?? null),
                },
                ocupadoPor: {
                    ...item.ocupadoPor,
                    conjuntoNombre: item.ocupadoPor.conjuntoId == null
                        ? null
...
function construirVentanasTrabajoDia(horario) {
    const bloqueos = buildBloqueosPorDescanso(horario).sort((a, b) => a.startMin - b.startMin);
    const ventanas = [];
...
function distribuirDuracionReordenamiento(params) {
    ...
    const cursorMin = (0, schedulerUtils_1.toMinOfDay)(inicioCursor);
    const ventanasOrdenadas = (0, schedulerUtils_1.mergeIntervalos)(ventanas).sort((a, b) => (a.i - b.i) || (a.f - b.f));
    for (let index = 0; index < ventanasOrdenadas.length; index += 1) {
        ...
        if (inicioSegmento >= ventana.f)
            continue;
        const disponibleActual = ventana.f - inicioSegmento;
        if (disponibleActual >= duracion) {
            return [
                {
                    fechaInicio: (0, schedulerUtils_1.toDateAtMin)(fecha, inicioSegmento),
                    fechaFin: (0, schedulerUtils_1.toDateAtMin)(fecha, inicioSegmento + duracion),
                },
            ];
        }
        const siguienteVentana = ventanasOrdenadas[index + 1];
        const puedeCruzarAlmuerzo = horario.descansoStartMin != null &&
            horario.descansoEndMin != null &&
            ventana.f === horario.descansoStartMin &&
            siguienteVentana?.i === horario.descansoEndMin;
        if (!puedeCruzarAlmuerzo)
            continue;
        const restante = duracion - disponibleActual;
        const disponibleSiguiente = (siguienteVentana?.f ?? 0) - (siguienteVentana?.i ?? 0);
        if (restante <= 0 || !siguienteVentana || disponibleSiguiente < restante)
            continue;
        return [
            {
                fechaInicio: (0, schedulerUtils_1.toDateAtMin)(fecha, inicioSegmento),
                fechaFin: (0, schedulerUtils_1.toDateAtMin)(fecha, ventana.f),
            },
            {
                fechaInicio: (0, schedulerUtils_1.toDateAtMin)(fecha, siguienteVentana.i),
                fechaFin: (0, schedulerUtils_1.toDateAtMin)(fecha, siguienteVentana.i + restante),
            },
        ];
    }
...
function construirVentanasOcupadasReordenamiento(params) {
    ...
    return (0, schedulerUtils_1.mergeIntervalos)(ocupadas).sort((a, b) => (a.i - b.i) || (a.f - b.f));
}
function buildTareaBorradorCreateData(original, fechaInicio, fechaFin) {
    ...
    return {
        ...
        insumosUsados: original.insumosUsados == null
            ? undefined
            : original.insumosUsados,
        ...
        operarios: original.operarios?.length
            ? {
                connect: original.operarios.map((operario) => ({
                    id: operario.id,
                })),
            }
            : undefined,
...
function construirMensajeSolapeReordenamiento(params) {
    ...
    const inicioSolape = new Date(Math.max(segmentoIntentado.fechaInicio.getTime(), tareaSolapada.fechaInicio.getTime()));
    const finSolape = new Date(Math.min(segmentoIntentado.fechaFin.getTime(), tareaSolapada.fechaFin.getTime()));
    const nombresOperarios = Array.from(new Set(tareaSolapada.operarios
        .map((operario) => operario.usuario?.nombre?.trim() || `Operario ${operario.id}`)
        .filter(Boolean)));
    const operariosTexto = nombresOperarios.length <= 1
    ...
    const tareaSolapadaTexto = tareaSolapada.descripcion?.trim() || `tarea ${tareaSolapada.id}`;
...
function formatHoraLocal(fecha) {
    return `${String(fecha.getHours()).padStart(2, "0")}:${String(fecha.getMinutes()).padStart(2, "0")}`;
}
function formatFechaLocal(fecha) {
    return `${String(fecha.getDate()).padStart(2, "0")}/${String(fecha.getMonth() + 1).padStart(2, "0")}/${fecha.getFullYear()}`;
}
function sameDayKeyLocal(fecha) {
    return `${fecha.getFullYear()}-${String(fecha.getMonth() + 1).padStart(2, "0")}-${String(fecha.getDate()).padStart(2, "0")}`;
}
function buildSugerenciaConflictoMaquinaria(params) {
    const { tipoSolape, tareaSolicitada, ocupadoPor } = params;
    const duracionMs = Math.max(60000, tareaSolicitada.usoFin.getTime() - tareaSolicitada.usoInicio.getTime());
    const baseFin = tipoSolape === "RESERVA_LOGISTICA"
    ...
    if (baseFin.getFullYear() >= 2099) {
        return {
            ...
            finUsoSugerido: null,
            nota: "La reserva ocupante no tiene fecha de cierre registrada. Revisa y cierra esa reserva antes de reprogramar.",
        };
    }
    ...
    return {
        ...
        nota: tipoSolape === "RESERVA_LOGISTICA"
...
function buildConflictoMaquinariaDetalle(params) {
    const { maquinariaId, maquinaNombre, tareaSolicitada, ocupadoPor, tipoSolape, motivo } = params;
    return {
        maquinariaId,
