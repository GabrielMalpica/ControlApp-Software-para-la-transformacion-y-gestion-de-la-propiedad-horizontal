...
class PlanEsperanzaService {
    constructor(client) {
        this.prisma = client ?? prisma_1.prisma;
    }
    async obtenerConfig(conjuntoId) {
    ...
    async actualizarConfig(conjuntoId, intervaloMeses) {
    ...
    async iniciarPlan(conjuntoId, mantenerEvidencias, planAnteriorId) {
        ...
        if (mantenerEvidencias && planAnteriorId) {
            ...
            const mapAnteriores = new Map(anteriores.map((d) => [d.elementoId, d]));
    ...
    async obtenerPlanActivo(conjuntoId) {
    ...
    async obtenerDiagnostico(diagnosticoId) {
    ...
    async obtenerHistorico(conjuntoId, planIds) {
        const hojas = await obtenerElementosHoja(this.prisma, conjuntoId);
        const planes = await this.prisma.planEsperanza.findMany({
            where: {
                conjuntoId,
    ...
    async reiniciarPlan(conjuntoId, mantenerEvidencias) {
    ...
    async verificarZonasNuevas(conjuntoId) {
        const planActivo = await this.prisma.planEsperanza.findFirst({
            where: { conjuntoId, completado: false },
            orderBy: { fechaInicio: "desc" },
        });
        if (!planActivo)
            return { hayZonasNuevas: false, zonasExistentes: 0, zonasActuales: 0 };
        const hojas = await obtenerElementosHoja(this.prisma, conjuntoId);
    ...
    async obtenerLineaTiempoElemento(elementoId) {
...
exports.PlanEsperanzaService = PlanEsperanzaService;
